        ...
        try {
        	...
        	switch (i) {
        		...
        		case 2:
        			...
        			messageAesKey = R.getMessageAesKey(); // 用服务器私钥解密
        			...
        			if(this.isValueEmptyOrNull(protectionAesKey)) { this.deliverResponse(response, "获取保护密码失败", this.GENERAL_ERROR_CODE, ""); return; }
        			
        			messageAesKey = frontEndCrypto.decryptByPrivateKey(messageAesKey, frontEndCrypto.SERVER_RSA_PRIVATE_KEY);
        			if(this.isValueEmptyOrNull(messageAesKey)) { this.deliverResponse(response, "解密通讯密码失败", this.GENERAL_ERROR_CODE, ""); return; }
        		...
        		case 3:
        			...
        			messageAesKey = frontEndCrypto.encryptByPublicKey(messageAesKey, clientRsaPublicKey);
        			if(this.isValueEmptyOrNull(messageAesKey)) { this.deliverResponse(response, "浏览器公钥加密通讯密码时出错", this.GENERAL_ERROR_CODE, ""); return; }
        		...
        		case 5:
        			...
        			if (this.isValueEmptyOrNull(encryptedText)){ this.deliverResponse(response, "加密通讯明文时出错", this.GENERAL_ERROR_CODE, ""); return; }
        		...
        		case 6:
        			...
        			if (this.isValueEmptyOrNull(plainText)){ this.deliverResponse(response, "解密通讯明文时出错", this.GENERAL_ERROR_CODE, ""); return; }
        			...
        			if (this.isValueEmptyOrNull(plainText)){ this.deliverResponse(response, "解密后加密通讯明文时出错", this.GENERAL_ERROR_CODE, ""); return; }
        			
        			protectionAesKey = frontEndCrypto.encryptByPublicKey(protectionAesKey, clientRsaPublicKey);
        			if (this.isValueEmptyOrNull(protectionAesKey)){ this.deliverResponse(response, "使用浏览器RSA公钥加密保护密码时出错", this.GENERAL_ERROR_CODE, ""); return; }
        		...
        		case 8:
        			...
        			plainText = frontEndCrypto.aesEncrypt(protectionAesKey, protectionAesKey, plainText);
        			if (this.isValueEmptyOrNull(plainText)){ this.deliverResponse(response, "使用保护密码加密通讯明文时出错", this.GENERAL_ERROR_CODE, ""); return; }
        			
        			protectionAesKey = frontEndCrypto.encryptByPublicKey(protectionAesKey, clientRsaPublicKey);
        			if (this.isValueEmptyOrNull(protectionAesKey)){ this.deliverResponse(response, "浏览器RSA公钥加密保护密码时出错", this.GENERAL_ERROR_CODE, ""); return; }
